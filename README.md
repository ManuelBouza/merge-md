...
- 🧹 **Limpieza automática:** borra los archivos temporales `.md` y `.html` generados después de abrir el navegador (configurable con `--keep`).
...
El resultado: el navegador se abrirá mostrando el documento HTML,
y los archivos temporales (`docs.md`, `docs.html`) serán eliminados automáticamente tras unos segundos.

---

### 📦 Mantener archivos generados

Si deseas conservar los archivos `.md` y `.html` generados:
...
| `main.sh`            | Coordina todo el proceso (merge → conversión → navegador → limpieza).  |
